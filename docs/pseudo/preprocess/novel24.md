...
이 데이터셋에서 Lance에 저장되는 row 형태는 `PT` 하나뿐이며, `messages`는 항상 `null`이다.
...
4. 모든 row의 `split`은 `"train"`으로 둔다.
5. 한 row의 `token_count`는 `1024`를 넘으면 안 된다.
...
6. 줄을 하나 더 넣은 후보 문자열의 token_count를 계산한다.
...
1. `data_type`은 `"PT"`로 둔다.
2. `source`는 `"novel24"`로 둔다.
3. `split`은 `"train"`으로 둔다.
4. `content`에는 현재 청크 문자열을 넣는다.
5. `messages`는 `null`로 둔다.
6. `token_count`는 현재 청크 문자열을 공통 토크나이저로 계산한 값으로 넣는다.
