...
다른 데이터와 달리 novel24는 train, val 디렉토리가 나뉘어져 있지 않다. 90:10으로 train:val을 구성한다. 이때 한 txt 파일 안을 90:10으로 나누는 것이 아닌 txt 파일 뭉치를 90:10으로 나눠 구성한다. 1,2,...10번 작품, 총 10개가 있을 때 1~9번 작품 train, 10번 작품 val로 구성한다. 1,2,...10번 작품 안의 90% train, 나머지 작품 일부 10%를 val로 구성하지 않는다.
