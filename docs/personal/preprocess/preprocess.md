...
minhash + LSH 중복 제거의 경우 content 필드의 String을 기준으로 한다. datatrove 라이브러리의 kiwi 기반 한국어 형태소 5-gram 으로 묶는다.
예를 들어 "산꼭대기를 향한 투쟁만으로도 인간의 마음을 채우기에 충분하다. 우리는 시지프가 행복하다고 상상하여야 한다." 라는 문장을 kiwi 기반 한국어 형태소 n-gram 으로 묶으면 "산꼭대기 를 향하 ㄴ 투쟁", "를 향하 ㄴ 투쟁 만", ..., "상상 하 여야 하 ㄴ다." 로 분해할 수 있다. 이러한 묶음의 집합을 가지고 hash 함수에 입력하여 minhash + LSH를 거치고 이 결과로 나온 유사한 것으로 추정되는 row들에 대해 Jaccard similarity를 계산하여 임계점(하이퍼파라미터로써 코드 맨 앞에 수정하기 쉬운 변수로 지정)을 넘으면 그 row들을 유사한 클러스터로 판단하고 대표 하나만을 남긴다. 대표를 결정하는 기준은 "REASONING" > "SFT" > "PT"이고 동률 시 content의 파이썬 기준 len이 긴 쪽을 우선한다. 즉, 귀한 추론 데이터와 파인튜닝 데이터를 우선하고 더 긴 텍스트를 선호한다는 의미이다. 보다 구체적인 흐름은 datatrove 라이브러리의 구현을 따른다.
exact, minhash + lsh dedup은 전체 데이터 소스를 섞어서 진행한다.
