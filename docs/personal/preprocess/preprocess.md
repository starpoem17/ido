...
minhash + LSH 중복 제거의 경우 content 필드의 String을 기준으로 한다. 공백을 제거한 한국어 문자 단위의 7-gram 으로 묶는다.
예를 들어 "산꼭대기를 향한 투쟁만으로도 인간의 마음을 채우기에 충분하다. 우리는 시지프가 행복하다고 상상하여야 한다." 라는 문장을 공백을 제거한 한국어 문자 단위의 7-gram으로 묶으면 "산꼭대기를향한", "꼭대기를향한투", ..., "상하여야한다." 로 분해할 수 있다. 이러한 묶음의 집합을 가지고 hash 함수에 입력하여 minhash + LSH를 거치고 이 결과로 나온 유사한 것으로 추정되는 row들에 대해 Jaccard similarity를 계산하여 임계점(하이퍼파라미터로써 코드 맨 앞에 수정하기 쉬운 변수로 지정)을 넘으면 그 row들을 유사한 클러스터로 판단하고 대표 하나만을 남긴다. 대표를 결정하는 기준은 "REASONING" > "SFT" > "PT"이고 동률 시 content의 파이썬 기준 len이 긴 쪽을 우선한다. 즉, 귀한 추론 데이터와 파인튜닝 데이터를 우선하고 더 긴 텍스트를 선호한다는 의미이다.
