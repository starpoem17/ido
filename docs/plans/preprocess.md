...
4. 전체 데이터 소스를 섞은 뒤, `content` 기준 한국어 문자 단위 5-gram shingle로 minhash + LSH 후보를 만들고 Jaccard similarity 임계값 기반으로 near-duplicate를 제거한다.
...
- minhash + LSH dedup은 `content` 문자열을 한국어 문자 단위 5-gram shingle 집합으로 바꾼 뒤 수행한다.
- minhash + LSH는 near-duplicate 후보를 찾는 단계로만 사용하고, 후보 row들에 대해서는 shingle 집합 기준 Jaccard similarity를 다시 계산한다.
